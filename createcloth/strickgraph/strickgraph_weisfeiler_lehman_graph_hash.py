...
:todo: rewrite to own function inside strickgraph_base
        replace string of edgetype and side and so on with configfilevalues
"""
...
def weisfeiler_lehman_graph_hash(
    ...
    """
    print("Thisfunction is obsolete is replaced by extrasfornetworkx" )
    edge_attr="edgetype"
    node_attr_type="stitchtype"
    node_attr_side="side"
    ...
    def neighborhood_aggregate(G, node, node_labels, edge_attr=None):
        ...
        if isinstance( G, networkx.MultiDiGraph ) and edge_attr:
            for nei in G.neighbors(node):
                tmphash = 0
                for i in range( len( G[node][nei] )):
                    tmphash = tmphash + G[node][nei][i][edge_attr].__hash__()
                prefix = repr(tmphash)
    ...
    for node in G.nodes():
        node_labels[node] = str(G.nodes[node][node_attr_type]) \
                            + str(G.nodes[node][node_attr_side])
