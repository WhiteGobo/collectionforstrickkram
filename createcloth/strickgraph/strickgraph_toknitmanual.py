...
def create_knit_commandarray( strickgraph ):
    ...
    #knitter = machine_knitter( stitch_needednooses, stitch_generatenooses, \
    #                                    dict_threads, strickgraph.get_rows() )
    #source = knitter_status( [], [], [], [], 0, knitter )
    #targets = list( knitter_status( stitches, [], [], [], i, knitter ) \
...
class machine_knitter_producer_commands:
        ...
        for st, req in self.stitches_with_requirements.items():
            ...
            if st not in knitted_stitches:
                if knitted_stitches.issuperset( predecessors ) \
                    ...
                    try:
                        if all( r==working_needle[i] for i,r in enumerate( working_needle_req ) ):
                            yield st
                    except IndexError:
                        pass
    ...
    def knit_stitch( self, status, stitchtype, stitchname=None ):
        ...
        if info["knitted_stitches"].issuperset( predecessors )\
            ...
            if all( r==info["working_needle"][i] for i,r in enumerate( working_needle_req ) ):
                info["working_needle"] = info["working_needle"][ len(working_needle_req): ]
                info["knitted_stitches"] = info["knitted_stitches"]\
                                            .union( [stitchname] )
                for new_noose in info["stitch_to_generated_nooses"][ stitchname ]:
    ...
    def load_noose( self, status, noose ):
        """
        ...
        info[ "saved_singlenooses" ] = info[ "saved_singlenooses" ]-{noose}
...
class machine_knitter_producer( machine_knitter_producer_commands ):
    ...
    def possible_commands( self, status ):
        ...
        #stitches_with_requirements, stitch_to_generated_nooses
        #threads, rows, knitted_stitches, saving_needle, working_needle, 
        #saved_singlenooses, current_thread 
    ...
    def minimal_distance( self, first, second ):
        ...
        missing_threads = [ i for i, t in first.threads.items() \
...
class machine_knitter:
    def __init__( self, stitches_with_requirements, stitch_to_generated_nooses, threads, rows ):
        """

        :param threads:
        :type threads: dict[ int, Iterable ]
        """
        self.stitches_with_requirements = stitches_with_requirements
        self.stitch_to_generated_nooses = stitch_to_generated_nooses
    ...
    def turn_needles( self, status ):
        knitted_stitches, saving_needle, working_needle, saved_singlenooses, \
                current_thread = status
        saving_needle, working_needle = working_needle, saving_needle
        return knitter_status( knitted_stitches, saving_needle, working_needle,\
                                    saved_singlenooses, current_thread, self )
    ...
    def knit_stitch( self, status, stitch ):
        knitted_stitches, saving_needle, working_needle, saved_singlenooses, \
                current_thread = status
        working_needle_req, predecessors, thread = self.stitches_with_requirements[ stitch ]
        if knitted_stitches.issuperset( predecessors )\
                            and thread == current_thread:
            if all( r==working_needle[i] for i,r in enumerate( working_needle_req ) ):
                try:
                    [ working_needle.pop(0) for a in working_needle_req ]
                except Exception as err:
                    raise Exception( working_needle_req, working_needle ) from err
                knitted_stitches.add( stitch )
                for new_noose in self.stitch_to_generated_nooses[ stitch ]:
                    saving_needle.insert( 0, new_noose )
            else:
                raise ValueError()
        else:
            raise ValueError()
    ...
    def save_noose( self, status ):
        ...
        knitted_stitches, saving_needle, working_needle, saved_singlenooses, \
                current_thread = status
        try:
            saved_singlenooses.add( working_needle.pop( 0 ) )
        except IndexError:
            raise
        return knitter_status( knitted_stitches, saving_needle, working_needle,\
                                    saved_singlenooses, current_thread, self )
    ...
    def load_noose( self, status, noose ):
        ...
        knitted_stitches, saving_needle, working_needle, saved_singlenooses, \
                current_thread = status
        try:
            saved_singlenooses.remove( noose )
        except KeyError:
            raise
        saving_needle.insert( 0, noose )
        return knitter_status( knitted_stitches, saving_needle, working_needle,\
                                    saved_singlenooses, current_thread, self )
    ...
    def change_thread( self, status, thread ):
        knitted_stitches, saving_needle, working_needle, saved_singlenooses, \
                current_thread = status
        return knitter_status( knitted_stitches, saving_needle, working_needle,\
                                    saved_singlenooses, thread, self )

class knitter_status:
    def __init__( self, knitted_stitches, saving_needle, working_needle, \
                            saved_singlenooses, current_thread, knitter ):
        """

        :param saving_needle:
        :type saving_needle: Iterable
        :param working_needle:
        :type working_needle: Iterable
        :param current_thread:
        :type current_thread: int
        """
        self.knitted_stitches = set( knitted_stitches )
        self.saving_needle = tuple( saving_needle )
        self.working_needle = tuple( working_needle )
        self.saved_singlenooses = set( saved_singlenooses )
        assert type(current_thread) == int
        self.current_thread = current_thread
        self.knitter = knitter

    def __iter__( self ):
        yield set( self.knitted_stitches )
        yield list( self.saving_needle )
        yield list( self.working_needle )
        yield set( self.saved_singlenooses )
        yield self.current_thread
    ...
    def mindist( self, other ):
        if self.knitted_stitches.difference( other.knitted_stitches ):
            raise method_Astar.TargetUnreachable()

        missing_stitches = other.knitted_stitches\
                            .difference( self.knitted_stitches )
        missing_threads = [ i for i, t in self.knitter.threads.items() \
                            if len(missing_stitches.union(t))>0 \
                            and i != self.current_thread ]
        missing_stitches_rows = [ missing_stitches.intersection(row) for row in self.knitter.rows ]
        row_to_need_noose = lambda row: it.chain.from_iterable( \
                            self.knitter.stitches_with_requirements[st][0] \
                            for st in row )
        missing_nooses_rows = [[ no for no in row_to_need_noose(row) \
                                if no not in self.working_needle ] \
                                for row in missing_stitches_rows ]
        missing_nooses_rows = [ row for row in missing_nooses_rows \
                                if len(row) > 0 ]
        switch_thread_cost = 0 if self.current_thread == other.current_thread \
                                else 1
        noose_switching_cost = abs( len(self.saved_singlenooses)- len(other.saved_singlenooses))
        return len( missing_stitches ) + switch_thread_cost + noose_switching_cost \
                + 2*len( missing_threads ) + len(missing_nooses_rows)

    def neighbours( self ):
        try:
            noose = self.working_needle[0]
            yield self.knitter.save_noose( self ), 3, ("save_noose", noose)
        except IndexError:
            pass
        for noose in self.saved_singlenooses:
            yield self.knitter.load_noose( self, noose ), 1, ("load_noose", noose)
        for st in self.knitter.knitable_stitch( self ):
            q = self.knitter.knit_stitch( self, st )
            yield q, 1, ("knit", st)
        for thread in self.knitter.threads:
            if thread != self.current_thread:
                q = self.knitter.change_thread( self, thread )
                yield q, 2, ("switch_thread", thread)
        turnstatus = self.knitter.turn_needles( self )
        if turnstatus != self:
            yield turnstatus, 1, ("turn", )
